...
void preprocess(cv::Mat& imgOriginal, cv::Mat& imgGrayscale, cv::Mat& imgThresh) {
    imgGrayscale = extractValue(imgOriginal); //  on exrtait la valeur de l'image original pour faire l'image avec grayscale

    cv::Mat imgMaxContrastGrayscale = maximizeContrast(imgGrayscale); // On va maximiser le contraste

    cv::Mat imgBlurred; // On créer une nouvelle image (qui va être flu)
    cv::GaussianBlur(imgMaxContrastGrayscale, imgBlurred, GAUSSIAN_SMOOTH_FILTER_SIZE, 0); // gaussian blur (FLU GAUSSIAN)
